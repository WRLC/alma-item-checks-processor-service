...
class SCFItemProcessor(BaseItemProcessor):
    ...
    def no_row_tray_process(self) -> None:
        ...
        logging.info(
            f"SCFItemProcessor.no_row_tray_process: Created entity with PartitionKey=scf_no_row_tray, RowKey={barcode}"
        )
        logging.info(
            f"SCFItemProcessor.no_row_tray_process: Table name: {SCF_NO_ROW_TRAY_STAGE_TABLE}"
        )
        logging.info(
            f"SCFItemProcessor.no_row_tray_process: Storage connection string configured: {STORAGE_CONNECTION_STRING is not None}"
        )
