...
class IZItemProcessor(BaseItemProcessor):
    ...
    def no_row_tray_report_process(self) -> bool:
        ...
        if updated:
            ...
            logging.warning(f"IZItemProcessor.no_row_tray_report_process: Failed to update IZ item {original_barcode}")
            return False
    ...
    def _handle_successful_update(self, item: Item, process_type: str) -> None:
        ...
        try:
            ...
            logging.error(f"IZItemProcessor._handle_successful_update: Failed to upload item data: {e}")
            return
        ...
        if not institution:
            logging.error(f"IZItemProcessor._handle_successful_update: Institution {institution_code} not found")
            return
        ...
        try:
            ...
            logging.error(f"IZItemProcessor._handle_successful_update: Failed to queue update message: {e}")
